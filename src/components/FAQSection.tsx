              ...
              return (
                <div key={idx} className="py-4">
                  ...
                  >
                    ...
                    <span className={`ml-auto grid h-7 w-7 place-items-center rounded-full border transition-all ${isOpen ? 'rotate-45 border-[#C29B72] bg-[#C29B72]/20 text-[#C29B72]' : 'border-white/40 bg-white/20 text-white/80'}`}>+
                    </span>
                  </button>
                  ...
                  <div id={`faq-panel-${idx}`} role="region" className={`grid transition-[grid-template-rows,opacity] duration-300 ease-out ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-70'}`}>
                    <div className="overflow-hidden">
                      <p className="mt-3 pr-2 text-sm leading-relaxed text-white/80">
                        {item.a}
                      </p>
                    </div>
                  </div>
